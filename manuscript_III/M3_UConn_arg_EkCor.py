...
newpath = r'/gpfs/sharedfs1/alb16177/vap24003/OutputData/20250725_Confine_CP_EkCor/'+case_name+'/'
...
# class vortex_track:
#     def __init__(self,mesh,Vor,mask,n_ti,t_set,*,vor_range:float=2.0,):
#         X, Y = mesh.x, mesh.y
#         if n_ti==0:
#             iniArr = np.zeros([len(t_set), 2])
#             self.pos = np.copy(iniArr)
#             self.vL = np.copy(iniArr)
#             self.K = np.zeros_like(t_set)}
#         pos1 = np.copy(np.argmax(Vor*mask))
#         pos2 = np.copy(np.argmin(Vor*mask))
#         self.pos[n_ti,:]  = np.copy(X[pos1]), np.copy(Y[pos1])
#         # Circulation of vortex and antivortex. vor_range = 2*xi
#         self.K[n_ti]  = circulation(mesh_input,Vs,self.pos[n_ti,:],vor_range)
#         # Velocity of vortex and antivortex.
#         if n_ti>0:
#             self.vL[n_ti,:]  = np.copy((self.pos[n_ti,:]-self.pos[n_ti-1,:])/(t_set[n_ti]-t_set[n_ti-1]))

def MaskMaker(mesh,*,edge:str='on',edge_Xwidth:float=0.025,edge_Ywidth:float=0.15,light:str='on',light_width:float=0.6,X0:float=0.0,Y0:float=0.0):
    X, Y = mesh.x, mesh.y
    mask = np.ones_like(X)
    maxX, maxY = max(X), max(Y)
    if  light.lower()=='on': # Mask of light spot:
        mask = mask - np.exp(-((X-X0)**2+(Y-Y0)**2)/light_width**2)
    if  edge.lower()=='on': # Mask of edge:
        mask[np.where(abs(X)>(1-edge_Xwidth)*maxX)] = 0.0
        mask[np.where(abs(Y)>(1-edge_Ywidth)*maxY)] = 0.0
    return mask

def Magnus_Force(density,circulation_z,vL,vs):
    FM = np.zeros([len(density),2])
    FM[:,1] = density*circulation_z*(vL[0]-vs[:,0]) - 0.0 # Fy = rho * (Kz*Vx - Kx*Vz)
    FM[:,0] = 0.0 - density*circulation_z*(vL[1]-vs[:,1]) # Fx = rho * (Ky*Vz - Kz*Vy)
    return FM

def Iordanskii_Force(density,circulation_z,vL,vn):
    # [ref]: https://iopscience.iop.org/article/10.1088/0022-3719/3/5/029/pdf
    FI = np.zeros([len(density),2])
    FI[:,1] = density*circulation_z*(vL[0]-vn[:,0]) - 0.0 # Fy = rho * (Kz*Vx - Kx*Vz)
    FI[:,0] = 0.0 - density*circulation_z*(vL[1]-vn[:,1]) # Fx = rho * (Ky*Vz - Kz*Vy)
    return FM

def add_f(mesh,F1,pos1,F2,pos2):
    X, Y = mesh.x, mesh.y
    mask_left = np.ones_like(F1)
    mask_left[np.where(X>0),:] = 0.0
    mask_right = 1.0-mask_left
    F_output = np.zeros_like(F1)
    if pos1[0]>=0 and pos2[0]<0:  F_output = F1*mask_right + F2*mask_left
    if pos1[0]<0 and pos2[0]>=0:  F_output = F1*mask_left  + F2*mask_right
    if pos1[0]>=0 and pos2[0]>=0: F_output = F1*mask_right + F2*mask_right
    if pos1[0]<0 and pos2[0]<0:   F_output = F1*mask_left  + F2*mask_left
    return F_output

def vs_value(mesh,vs,pos,vor_range,int_range):
    distance = np.sqrt((mesh.x-pos[0])**2+(mesh.y-pos[1])**2)
    S_integral = np.zeros_like(mesh.x)
    S_integral[np.where(distance<=(vor_range+int_range))] = 1.0
    S_integral[np.where(distance<=vor_range)] = 0.0
    return [sum(S_integral*vs[:,0]*mesh.areas)/sum(S_integral*mesh.areas), sum(S_integral*vs[:,1]*mesh.areas)/sum(S_integral*mesh.areas)]

def rv_value(mesh,density,vs,vn,vL,pos,vor_range,int_range):
    distance = np.sqrt((mesh.x-pos[0])**2+(mesh.y-pos[1])**2)
    S_integral = np.zeros_like(mesh.x)
    S_integral[np.where(distance<=(vor_range+int_range))] = 1.0
    S_integral[np.where(distance<=vor_range)] = 0.0
    int_area = sum(S_integral*mesh.areas)
    rvL = [sum(S_integral*density*vL[0]*mesh.areas)/int_area, sum(S_integral*density*vL[1]*mesh.areas)/int_area]
    rvs = [sum(S_integral*density*vs[:,0]*mesh.areas)/int_area, sum(S_integral*density*vs[:,1]*mesh.areas)/int_area]
    rvn = [sum(S_integral*(1-density)*vn[:,0]*mesh.areas)/int_area, sum(S_integral*(1-density)*vn[:,1]*mesh.areas)/int_area]
    rho = sum(S_integral*density*mesh.areas)/int_area
    return rvL, rvs, rvn, rho

def FM_value(mesh,density,circulation_z,vL,vs,pos,vor_range,int_range):
    # Magnus force
    FM = np.zeros([len(density),2])
    FM[:,1] = density*circulation_z*(vL[0]-vs[:,0]) - 0.0 # Fy = rho * (Kz*Vx - Kx*Vz)
    FM[:,0] = 0.0 - density*circulation_z*(vL[1]-vs[:,1]) # Fx = rho * (Ky*Vz - Kz*Vy)
    # Integral for a value
    distance = np.sqrt((mesh.x-pos[0])**2+(mesh.y-pos[1])**2)
    S_integral = np.zeros_like(mesh.x)
    S_integral[np.where(distance<=(vor_range+int_range))] = 1.0
    S_integral[np.where(distance<=vor_range)] = 0.0
    int_area = sum(S_integral*mesh.areas)
    return [sum(S_integral*FM[:,0]*mesh.areas)/int_area, sum(S_integral*FM[:,1]*mesh.areas)/int_area]


def FMtrack(rec,mesh,density,Vor,vs,vn,mask,n_ti,t_set,t_rec,*,vor_range:float=2.0,int_range:float=0.5,):
    X, Y = mesh.x, mesh.y
    if n_ti==0:
        iniArr = np.zeros([len(t_set), 2], dtype=float)
        rec['t'] = np.zeros_like(t_set, dtype=float)

        rec['V']  = {'pos':np.copy(iniArr),'vL':np.copy(iniArr),'vs':np.copy(iniArr),'vn':np.copy(iniArr),
                     'K':np.zeros_like(t_set, dtype=float),'FM':np.copy(iniArr),'FI':np.copy(iniArr),
                     'rvL':np.copy(iniArr),'rvs':np.copy(iniArr),'rvn':np.copy(iniArr),
                     'rho':np.zeros_like(t_set, dtype=float)}
        rec['aV'] = {'pos':np.copy(iniArr),'vL':np.copy(iniArr),'vs':np.copy(iniArr),'vn':np.copy(iniArr),
                     'K':np.zeros_like(t_set, dtype=float),'FM':np.copy(iniArr),'FI':np.copy(iniArr),
                     'rvL':np.copy(iniArr),'rvs':np.copy(iniArr),'rvn':np.copy(iniArr),
                     'rho':np.zeros_like(t_set, dtype=float)}
    pos1 = np.copy(np.argmax(Vor*mask)) # for vortex
    pos2 = np.copy(np.argmin(Vor*mask)) # for antivortx
    rec['t'][n_ti] = t_rec
    rec['V']['pos'][n_ti,:]  = np.copy(X[pos1]), np.copy(Y[pos1])
    rec['aV']['pos'][n_ti,:] = np.copy(X[pos2]), np.copy(Y[pos2])
   # Circulation of vortex and antivortex. vor_range = 2*xi
    rec['V']['K'][n_ti]  = circulation(mesh,Vor,rec['V']['pos'][n_ti,:],vor_range)
    rec['aV']['K'][n_ti] = circulation(mesh,Vor,rec['aV']['pos'][n_ti,:],vor_range)
   # integral of vs around vortex and antivortex
    rec['V']['vs'][n_ti,:]  = vs_value(mesh,vs,rec['V']['pos'][n_ti,:],vor_range,int_range)
    rec['aV']['vs'][n_ti,:] = vs_value(mesh,vs,rec['aV']['pos'][n_ti,:],vor_range,int_range)
   # integral of vn around vortex and antivortex
    rec['V']['vn'][n_ti,:]  = vs_value(mesh,vn,rec['V']['pos'][n_ti,:],vor_range,int_range)
    rec['aV']['vn'][n_ti,:] = vs_value(mesh,vn,rec['aV']['pos'][n_ti,:],vor_range,int_range)
   # Velocity of vortex and antivortex.
    if n_ti<0:
        rec['V']['vL'][n_ti,:]  = np.copy((rec['V']['pos'][n_ti,:]-rec['V']['pos'][n_ti-1,:])/(t_set[n_ti]-t_set[n_ti-1]))
        rec['aV']['vL'][n_ti,:] = np.copy((rec['aV']['pos'][n_ti,:]-rec['aV']['pos'][n_ti-1,:])/(t_set[n_ti]-t_set[n_ti-1]))
        rec['V']['rvL'][n_ti,:],  rec['V']['rvs'][n_ti,:],  rec['V']['rvn'][n_ti,:],  rec['V']['rho'][n_ti]  = rv_value(mesh,density,vs,vn,rec['V']['vL'][n_ti,:],rec['V']['pos'][n_ti,:],vor_range,int_range)
        rec['aV']['rvL'][n_ti,:], rec['aV']['rvs'][n_ti,:], rec['aV']['rvn'][n_ti,:], rec['aV']['rho'][n_ti] = rv_value(mesh,density,vs,vn,rec['aV']['vL'][n_ti,:],rec['aV']['pos'][n_ti,:],vor_range,int_range)
   # Magnus force
    rec['V']['FM'][n_ti,:]  = FM_value(mesh,density,rec['V']['K'][num_set],rec['V']['vL'][num_set],vs,rec['V']['pos'][n_ti,:],vor_range,int_range)
    rec['aV']['FM'][n_ti,:] = FM_value(mesh,density,rec['aV']['K'][num_set],rec['aV']['vL'][num_set],vs,rec['aV']['pos'][n_ti,:],vor_range,int_range)
   # Iordanskii force
    rec['V']['FI'][n_ti,:]  = FM_value(mesh,(1-density),rec['V']['K'][num_set],rec['V']['vL'][num_set],vn,rec['V']['pos'][n_ti,:],vor_range,int_range)
    rec['aV']['FI'][n_ti,:] = FM_value(mesh,(1-density),rec['aV']['K'][num_set],rec['aV']['vL'][num_set],vn,rec['aV']['pos'][n_ti,:],vor_range,int_range)
    return rec
...
#         psi_plot[:,findval(ti_show,ti)] = np.array(psi_interp[py,:])
#         phase_plot[:,findval(ti_show,ti)] = np.array(phase_interp[py,:])
#         vor_plot[:,findval(ti_show,ti)] = np.array(vorticity_ti[py,:])
#         J_plot[:,findval(ti_show,ti)] = np.array(J_ti[py,:])
#         E_x, E_y = E_input_frame(Xv,Yv,ti,take_real=False)
#         B_x, B_y, B_z = E2Bv(Xv,Yv,E0i*E_x,E0i*E_y,constant_Bz,c,w_input)
...
# psi_plot, phase_plot, vor_plot, J_plot, PhiB_plot, fig = scan_value(501)
# fig.show()
...
# # frame_set = frame_set[0+312:]

# iniframe = frame_set[0]

# # ti_set = [1,2,3]
# ti_set = np.linspace(0, time.max(), num=501)
...
# # ax1.set_ylabel('$E_{x}$', color=color)
# ax1.plot(time[iniframe:], 1*np.cos(time[iniframe:]*w_input), color=color, alpha=0.5, label='norm($E_x$)')
# # ax1.tick_params(axis='y', labelcolor=color)
# ax1.set_ylim([-1.4,1.4])
# ax1.set_xlim([0,solve_time])
# ax1.legend(loc='center left', bbox_to_anchor=(1, 0.5))
# ax1.set_yticks([-1,1])
# ax1.set_title('$E_0$: '+str(E0i)+', $I$: '+str(current_amp))
...
#                 if i==3: Z_value = np.copy(J)/J_plot.max() #K0
...
#                 # axn.text(-28,18,label, fontsize=labelfontsize, color=labeltextcolor)
#                 # collections.append(collection)
#             plt.savefig(folder_name+'/'+str(num_set)+'.png')
